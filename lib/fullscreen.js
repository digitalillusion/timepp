...
var Fullscreen = class Fullscreen {
    constructor (monitor) {
        ...
        this.top_box_left.add_child(this.monitor_button);
        let monitor_icon = new St.Icon({ gicon : MISC.get_icon('timepp-monitor-symbolic') });
        ...
        let close_icon = new St.Icon({ gicon : MISC.get_icon('timepp-close-symbolic') });
        ...
        this.banner_container.connect('notify::allocation', () => {
            if (this.is_open && !this.banner_container_handler_block) {
                // @HACK If we don't do this in via idle_add then the
                // fullscreen ui loses focus and we start leaking clicks.
                Mainloop.idle_add(() => this._fit_banner());
            }
        });
        ...
        this.actor.connect('enter-event', () => {
            if (this.display.get_current_monitor() !== this.monitor) this.actor.grab_key_focus();
        });
        this.actor.connect('key-press-event', (_, event) => {
            ...
            if (symbol === Clutter.KEY_Escape) {
                ...
                if (event.get_state() === Clutter.ModifierType.CONTROL_MASK) direction = Gtk.DirectionType.TAB_BACKWARD;
    ...
    open () {
        if (this.is_open) return;
        this.is_open = true;
        this.modal = Main.pushModal(this.actor);
    ...
    _update_banner_size () {
        ...
        let border_width = Math.floor((banner_container_w - (banner_container_w * this.banner_size)) / 2);

        this.banner_container.set_style(`padding: 0 ${border_width}px;`);
    ...
    _fit_banner (caller) {
        ...
        while (true) {
            ...
            if (curr_state !== prev_state) {
                ...
                if (modifier === 1) {
                    if (curr_state) label.set_style('font-size: ' + (font_size - 2) + 'px;');
                    break;
                }
            }
            ...
            label.set_style('font-size: ' + font_size + 'px;');
