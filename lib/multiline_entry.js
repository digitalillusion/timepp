...
var MultiLineEntry = class MultiLineEntry {
    constructor (hint_text, scrollable, single_line_mode) {
        ...
        if (scrollable) {
            this.scroll_box = new St.ScrollView({ y_align: St.Align.START, style_class: 'multiline-entry-scrollbox vfade'});
        ...
        this.entry = new St.Entry({ can_focus: true, hint_text: hint_text });
        ...
        this.entry.connect('notify::allocation', () => {
            ...
            Mainloop.idle_add(() => this._resize_entry());
        });
